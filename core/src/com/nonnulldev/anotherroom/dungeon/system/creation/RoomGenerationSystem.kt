...
import com.nonnulldev.anotherroom.dungeon.data.Room
import com.nonnulldev.anotherroom.dungeon.enum.DungeonTileTypes
import com.nonnulldev.anotherroom.dungeon.enum.RoomSize
import java.util.*

class RoomGenerationSystem(private val dungeonCreationObject: DungeonCreationObject) : EntitySystem() {
    ...
    private fun addRoom(room: Room) {
        ...
        for (roomX in 0..room.dimension.width - 1) {
            for (roomY in 0..room.dimension.height - 1) {
                val dungeonTile = dungeonCreationObject.grid[room.coordinates.x + roomX][room.coordinates.y + roomY]
                dungeonTile.regionId = regionId
                dungeonTile.type = DungeonTileTypes.Room
            }
        }
        dungeonCreationObject.rooms.add(room)
    ...
    private fun canBuildRoom(room: Room): Boolean {
        ...
        for (roomX in 0..roomWidthWithWalls) {
            for (roomY in 0..roomHeightWithWalls) {
                var tile = dungeonCreationObject.grid[room.coordinates.x + roomX - (GameConfig.WALL_SIZE.toInt() * 2)][room.coordinates.y + roomY - (GameConfig.WALL_SIZE.toInt() * 2)]
